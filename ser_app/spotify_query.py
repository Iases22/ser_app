...
class SpotifyAPI(object):
    ...
    def get_token_headers(self):
        ...
        return {
            "Authorization":
            f'Basic {client_creds_b64.decode()}'  # <base64 encoded client_id:client_secret >
        }
    ...
    def get_resource(self, lookup_id, resource_type='albums', version='v1'):
        ...
        if r.status_code not in range(
                200,
                299):  # returns a boolean value. checking http stauts reponse
            return {'No results'}
        return r.json()
    ...
    def perform_auth(self):
        ...
        expires_time_left = current_datetime + datetime.timedelta(
            seconds=expires_in)
...
def get_spotify_links(emotion):
    ...
    playlist_ids = {
        'sad': '4GBXXvhMRpTpIxw6ekgRzM',
        'angry': '7g3ldE2qrypfcaCMuGr1uO',
        'happy': '2w1JTNzciFssDDVmbUVLS0',
        'disgust': '3yUY8lqZCcCyQUJwulRYQW',
        'fearful': '47KQmXAH8nwKMDv4e8hbcc',
        'calm': '78SqcmYkpBdihtP2uJTAeD'
    }
    ...
    spotify_artist = []
    for i in range(5):
        spotify_artist.append(
            spotify.get_playlist(playlist_ids[emotion]).json()['tracks']
            ['items'][i]['track']['album']['artists'][0]['name'])
        spotify_urls.append(
            spotify.get_playlist(playlist_ids[emotion]).json()['tracks']
            ['items'][i]['track']['external_urls']['spotify'])
        spotify_tracknames.append(
            spotify.get_playlist(playlist_ids[emotion]).json()['tracks']
            ['items'][i]['track']['name'])
    return spotify_artist, spotify_tracknames, spotify_urls
